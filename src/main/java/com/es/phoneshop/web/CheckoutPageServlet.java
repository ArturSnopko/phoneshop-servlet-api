...
public class CheckoutPageServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        setRequiredParameter(request, FIRST_NAME, errors, order::setFirstName, val -> val.length() >= 2);
        setRequiredParameter(request, LAST_NAME, errors, order::setLastName, val -> val.length() >= 2);
        setRequiredParameter(request, PHONE, errors, order::setPhone,val -> val.matches("^\\+?[0-9\\-\\s]{7,15}$"));
